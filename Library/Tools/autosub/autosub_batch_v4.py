...
if sys.platform == "win32":
    ...
    def _patched_popen(*args, **kwargs):
        ...
        kwargs['creationflags'] = cflags | subprocess.CREATE_NO_WINDOW
...
_VENV_PY = os.path.join(CURRENT_DIR, ".venv", "bin", "python" if sys.platform != "win32" else "python.exe")
PYTHON_EXE = _VENV_PY if os.path.exists(_VENV_PY) else sys.executable

# Core Tool Commands
# Note: These point to their absolute locations to avoid PATH issues
...
class HardwareManager:
    ...
    def _load_or_detect(self):
        ...
        try:
            ...
            has_cuda = bool(ctypes.windll.kernel32.GetModuleHandleW("nvcuda.dll"))
        except: pass
        ...
        br_max = 1
        if has_cuda:
            # If we have a dedicated GPU, we can handle more parallel work
            tr_max = 2
            br_max = 2 
...
class ResourceManager:
    ...
    def __init__(self, config_dir: str):
        ...
        self.io_sem = threading.Semaphore(self.hw.get("io_max", 6))       # Download & Merge
...
class BatchEngine:
    ...
    def get_video_duration(self, file_path: str) -> float:
        ...
        try:
            ...
            out = subprocess.check_output(cmd, creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0).decode('utf-8').strip()
            return float(out)
        except: return 0.0
    ...
    def fetch_metadata(self, url: str, cookie_path: Optional[str] = None):
        ...
        try:
            ...
            if os.path.exists(cookie_path):
                with use_temp_cookies(cookie_path) as temp_cookies:
                    if temp_cookies:
                        cmd.extend(["--cookies", temp_cookies])
                        out = subprocess.check_output(cmd, creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0).decode('utf-8', errors='ignore')
                    else:
                        out = subprocess.check_output(cmd, creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0).decode('utf-8', errors='ignore')
            else:
                out = subprocess.check_output(cmd, creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0).decode('utf-8', errors='ignore')
            ...
            for entry in data['entries']:
                ...
                # Sanitize title: remove newlines and multiple spaces
                v_title = re.sub(r'[\r\n\t]+', ' ', v_title).strip()
                if v_id and v_title:
                    results.append({
                        "title": v_title,
            ...
            try:
                ...
                if stage == "DL":
                    ...
                    if srts:
                        ...
                        style = self.settings.get("style", "auto")
                    ...
                    if bi_srt and raw_vid:
                        ...
                        ass_cmd = list(SRT2ASS_CMD) + [s_path, a_path]
                    ...
                    if os.path.exists(sync_script):
                        ...
                        cmd = [PYTHON_EXE, sync_script, "--local-dir", sync_dir, "--remote-id", folder_id, "--wrap-folder", "--headless"]
